...
The legacy routes remain available under `/categorys`; new category-specific actions are also mounted under `/categories`.
...
- `POST /categories/:id/allocate-unallocated`
  Supports explicit dollar allocations:
  ```json
  { "allocations": [{ "item_id": 7, "amount": 25.5 }] }
  ```
  Or percentages of the category's unallocated balance for active, not-yet-funded items:
  ```json
  { "percentages": [{ "item_id": 7, "percentage": 50 }] }
  ```
  The endpoint validates category ownership, active target items, unallocated-balance limits, and item target limits before updating item balances and the category unallocated balance in one transaction. Fully funded items create allocation log and notification records.
...
- `category` - `category_id`, `name` (45 chars), `description` (200 chars), `unallocated_balance` (double, default 0), `user_id` (FK, cascade delete).
- `item` - `item_id`, `name` (45 chars), `description`, `cost` (double), `balance` (double, default 0), `category_id` (FK), `user_id` (FK).
- `allocation_log` - transactional records for unallocated fund allocations and fully funded item events.
- `notification` - user-facing notification records, including fully funded item alerts.

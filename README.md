...
- **Categories:** containers (e.g., _Travel_, _Electronics_, _Needs_).
- **Mini‑accounts (items):** specific goals inside categories (e.g., _Spain trip_, _AirPods_, _Rent_).
...
> **Status:** Core CRUD for _categories/items_ and _transfers_ is implemented. Bank connections, wage projections, priority scheduling, and automated allocation are **planned** features.
...
- Email/password registration, login, logout, forgot-password, reset-password, and session checks backed by bcrypt hashing and JWTs.
- Passwords are validated on signup, password change, and password reset: minimum 8 characters with at least 1 letter and 1 number.
- Access tokens (15 minutes) issued in responses and kept in frontend memory only; refresh tokens (1 day) are stored in MySQL and sent as httpOnly cookies. Tokens are not stored in localStorage.
...
- Google OAuth is omitted from the MVP so the initial release can focus on email/password auth, httpOnly cookie sessions, and password recovery before adding third-party OAuth consent-screen and callback configuration.
...
- `generateAccessToken` signs user payloads with `ACCESS_TOKEN` and expires in 15 minutes. The React app stores this token in memory, not in `localStorage` or `sessionStorage`.
...
- Login and session checks only allow users where `deleted_at IS NULL`; account deletion soft-deletes users and clears their refresh token.
- Password rules are shared by the backend auth endpoints: minimum 8 characters, at least 1 letter, and at least 1 number.
- Forgot-password uses `POST /auth/forgot-password` to create a secure random reset token, stores only a SHA-256 token hash in `password_reset_token`, and expires tokens after 60 minutes. For the MVP, no email provider is configured; when `NODE_ENV=development`, the backend logs the raw reset token to the server console for local testing only.
- Reset-password uses `POST /auth/reset-password` with the raw token and a valid new password. On success, the password is re-hashed, the reset token is marked used, and existing refresh sessions for that user are cleared.
- Google OAuth is omitted for MVP because the project currently prioritizes first-party email/password auth, secure cookie-based sessions, and password recovery. Adding OAuth later will require provider credentials, consent-screen setup, callback URLs, and account-linking decisions.
...
- `POST /auth/forgot-password`
  ...
  ```
  Always returns a generic success message when the request can be processed. In development only, the backend logs the reset token instead of sending email.
- `POST /auth/reset-password`
  ```json
  { "token": "raw-reset-token", "password": "newpass1" }
  ```
...
- `POST /users/login`
  ```json
  { "user": { "email": "demo@example.com", "password": "secret123" } }
  ```
...
- `user` - `user_id`, `email` (unique), `password` (bcrypt hash), `f_name`, `l_name`, `refresh_token`, `deleted_at` for soft deletes.
- `password_reset_token` - secure password-reset token hashes, expiry timestamps, used timestamps, and the owning `user_id`.
